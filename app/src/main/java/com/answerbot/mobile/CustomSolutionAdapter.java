...
public class CustomSolutionAdapter extends ArrayAdapter<Result> {
    ...
    public CustomSolutionAdapter(Context context, ArrayList<Result> placeHolders) {
        super(context, com.answerbot.mobile.R.layout.item_result, placeHolders);
    }

    @Override
    public View getView(int position, View convertView, ViewGroup parent) {
        if(convertView==null){
            convertView= LayoutInflater.from(getContext()).inflate(com.answerbot.mobile.R.layout.list_result,parent,false);
        }
        Result result = getItem(position);

        Typeface tf = Typeface.createFromAsset(getContext().getAssets(), "RobotoCondensed-Regular.ttf");

        TextView show_result = (TextView) convertView.findViewById(com.answerbot.mobile.R.id.result_item);
